...
def conn_handler(connection, buff_size, addr, conn_timeout, num_workers, log):
    ...
    if (threading.active_count() > num_workers) :
        ...
        connection.close()
        return
    #if countWorkers >= num_workers:
    #   print "All workers are busy"
    #   print("countWorkers %d numWorkers %d" %(countWorkers, num_workers))
    #   print "Closing the browser connection"
    #   connection.close()
    #   return
    try:
        ###HERE
...
def s_proxy(webserver, port, client_conn, addr, data, reqindex,timeout) :
